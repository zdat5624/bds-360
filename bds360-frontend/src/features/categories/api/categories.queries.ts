...
import { Category, CategoryFilterRequest } from './types';

export const CATEGORIES_QUERY_KEYS = {
    all: ['categories'] as const,
    lists: () => [...CATEGORIES_QUERY_KEYS.all, 'list'] as const,
    list: (filters: CategoryFilterRequest) => [...CATEGORIES_QUERY_KEYS.lists(), filters] as const,
...
const getCategories = async (filters: CategoryFilterRequest): Promise<PageResponse<Category>> => {
    return customFetch.get('/categories', { params: filters });
};

// Gọi riêng khi cần danh sách cho trang quản trị (Admin)
const getAdminCategories = async (filters: CategoryFilterRequest): Promise<PageResponse<Category>> => {
...
export const useGetCategories = (filters: CategoryFilterRequest, isAdmin: boolean = false) => {
    return useQuery({
